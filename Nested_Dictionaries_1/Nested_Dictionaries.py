...
def iterateDictionary(some_list):
    for s in some_list:
        output = ''
        for key, value in s.items():  # .item method iterates over the key-value pairs in a dictionry
            output += f"{key} - {value} "
        print(output)
...
def iterateDictionary2(key_name, some_list):
    for items in range(len(some_list)):
        print(some_list[items][key_name])
